...
export const useTheme = () => {
  ...
  const { Variables: darkThemeConfigVars = {}, ...darkThemeConfig } = darkMode ? themes[`${currentTheme}_dark`] || {} : {};

  const themeVariables = useMemo(() => mergeVariables(DefaultVariables as ThemeVariables, themeConfigVars, darkThemeConfigVars), [themeConfigVars, darkThemeConfigVars]);
  ...
  const formattedTheme = useMemo(() => formatTheme(themeVariables, themeConfig || {}), [themeVariables, themeConfig]);

  const formattedDarkTheme = useMemo(() => formatTheme(themeVariables, darkThemeConfig || {}), [themeVariables, darkThemeConfig]);

  const generatedTheme = useMemo(() => buildTheme(darkMode, baseTheme, formattedTheme, formattedDarkTheme), [darkMode, baseTheme, formattedTheme, formattedDarkTheme]);
...
const mergeVariables = (variables: ThemeVariables, themeConfig: Partial<ThemeVariables>, darkThemeConfig: Partial<ThemeVariables>): ThemeVariables =>
...
const buildTheme = (darkMode: boolean, baseTheme: Theme, themeConfig: Partial<Theme>, darkThemeConfig: Partial<Theme>) => {
  return {
    ...
    darkMode,
    NavigationTheme: darkMode ? ReactNavigationDarkTheme : ReactNavigationDefaultTheme,
  };
};
