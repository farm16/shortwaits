...
// @ts-ignore
const paymentMethodSchema: Yup.Schema<EventPaymentMethodType> = Yup.string()
  .required()
  .oneOf(Object.keys(eventPaymentMethods));

export const createEventSchema: Yup.Schema<CreateEventDtoType> =
  Yup.object().shape({
...
export const updateEventSchema: Yup.Schema<UpdateEventDtoType> =
  Yup.object().shape({
    paymentMethod: paymentMethodSchema,
    participantsIds: Yup.array().of(Yup.string()),
    urls: Yup.array().of(
