...
export const createServiceSchema: (config: any) => Yup.SchemaOf<CreateServiceDtoType> = config =>
  Yup.object().shape({
...
export const updateServiceSchema: (config: any) => Yup.SchemaOf<CreateServiceDtoType> = config =>
  Yup.object().shape({
    businessId: Yup.string(),
    name: Yup.string().min(3, "a longer name is required").required("this field is required"),
    description: Yup.string().optional(),
