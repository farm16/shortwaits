...
import { BackButton, Button, ButtonCard, CurrencyFieldCard, FormContainer, IconButton, Space, Text, TextFieldCard, TimePickerFieldCard } from "@shortwaits/shared-ui";
...
export const UpdateEventModal: FC<ModalsScreenProps<"update-event-modal-screen">> = ({ navigation, route }) => {
  ...
  const [selectedService, setSelectedService] = useState<ServiceDtoType | null>(services.find(service => service._id === (initialValues as EventDtoType).serviceId) || null);
  ...
  return createEventStatus.isLoading ? (
    ...
    <FormContainer footer={renderSubmitButton}>
      ...
      <CurrencyFieldCard
        ...
        currencyType={"USD"}
      />
      <TextFieldCard title="Notes" multiline value={values?.notes} onChangeText={handleChange("notes")} isTouched={touched.notes} errors={errors.notes} />
      <Space size="large" />
    </FormContainer>
  );
};
